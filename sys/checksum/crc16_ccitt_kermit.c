...
/**
 * @ingroup     sys_checksum_crc16_ccitt_kermit
 * @{
 *
 * @file
 ...
 * @author      Lammert Bies <info@lammertbies.nl>
...
static const uint16_t _crc16_lookuptable_kermit[256] = {
...
uint16_t crc16_ccitt_kermit_update(uint16_t crc, const unsigned char *buf, size_t len) {
    ...
    crc = byteorder_swaps(crc);

    while(len--) {
        short_c = 0x00ff & (unsigned short) *buf++;

        tmp =  crc       ^ short_c;
        crc = (crc >> 8) ^ _crc16_lookuptable_kermit[tmp & 0xff];
    }

    return byteorder_swaps(crc);
}

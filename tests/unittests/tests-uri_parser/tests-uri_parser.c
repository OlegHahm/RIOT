                                  ...
                                  const char *actual_str, size_t actual_len)
{
    ...
    printf("\nWith given input uri '%s', expected %s '%s' with length '%d' but got '%.*s' with "
           "length '%d'\n", input_uri, name, expected_str, (unsigned)strlen(expected_str),
           (unsigned)actual_len, actual_str, (unsigned)actual_len);
    return -1;
}
    ...
    for (unsigned int i = 0; i < ARRAY_SIZE(test_vec); ++i) {
        ...
        if (ret != expected_ret) {
            ...
            if (uri_res.scheme_len != strlen(test_vec[i]->scheme)) {
                printf("With given input uri '%s', expected a scheme with the length '%d'"
                       " but got '%d'\n", test_vec[i]->input_uri,
                       (unsigned)strlen(test_vec[i]->scheme), uri_res.scheme_len);
                TEST_FAIL(failure_msg);
            }
            else {
